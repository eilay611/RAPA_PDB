...
def plot_boxplot(seriesX, seriesY, seriesHue=None,
                ...
                palette=None, x_order=None, xy_title_fontsize=14,
                 add_mean=False,
                 mean_marker='_', mean_color='red',
                 mean_size=100, mean_linewidth=3, mean_alpha=1,
                 hide_indices_in_stripplot=None):
    # * hide_indices_in_stripplot - not all indices must be contained in df
    ...
    if seriesHue is not None: # hue exists
        ...
        if stripplot:
            sns.stripplot(data=stripplot_data, x=seriesX.name, y=seriesY.name,
                          ...
                          palette=stripplot_palette, order=x_order, split=True)
    else:                     # no hue
        if boxplot:
        ...
        if stripplot:
            sns.stripplot(data=stripplot_data, x=DataTools.get_col_name(seriesX),
                          ...
                          size=stripplot_size, color=stripplot_color,
                          palette=stripplot_palette, order=x_order)
...
def plot_clustermap(numbersTable, cmap='YlGnBu', figsize=(8, 8),
                    ...
                    cbar_title='', cbar_orient='vertical', cbar_pos=None,
    ...
    :param cbar_pos: colormap position
...
def plot_barplot_from_series(counts, figsize=(18, 8), title='',  ylabel='', xrotation=90, annot=True):
    """
    ...
    if annot:
        ...
        for i in range(len(y_vals)):
            plt.annotate("{:.1f}".format(y_vals[i]), xy=(x_vals[i], y_vals[i]),
...
def spaghetti_patients(patientDf, classColumn=None, figsize=(6, 8),
    ...
    if ((classColumn is not None) & (showLegend == True)):
        ...
        legendColors = []
        for c in range(len(legendLabels)):
            legendColors.append(Line2D([0], [0],
                                       color=colorsCat['mapper'].get(legendLabels[c]),
...
def plot_scatter_hue(series_x, series_y, series_hue=None,
    ...
    if y_title== '':
        y_title = DataTools.get_col_name(series_y)
    if legendTitle=='' and series_hue is not None:
        legendTitle = DataTools.get_col_name(series_hue)
    ...
    if save_folder is not None:
        ...
        if series_hue is not None: fileName = fileName + ' BY ' + legendTitle + '.jpg'
...
def parallele_coordinates_plot(df, class_column=None, cmap='Set1', figsize=(8, 5),
    ...
    plt.figure(figsize=figsize)
    ax = parallel_coordinates(df, class_column, colormap=plt.get_cmap(cmap),
                         alpha=line_alpha)
    ...
    if plot_markers:
        cluster_color_list = get_colors_4_categorical_series(df[class_column],
                                                             cmap=cmap)['colorSeries']
...
def plot_table(df, row_colors_list=None, col_widths=None):
    fig, ax = plt.subplots()
...
def add_horizontal_line_to_ax(height, ax, start_x, end_x,
    ...
    # if start_x and end_x aren't defined, look for the ticks of
    # start_xticklabel and end_xticklabel, and define start_x and end_x
    # accordingly.
    if start_x is None and end_x is None:
        if start_xticklabel is None and end_xticklabel is None:
            ...
            # for tick, ticklabel in enumerate(ax.get_xticklabels()):
        ...
        if find_closest_labels: # look for the first ticklabel with number larger than start_xticklabel
            for tick, ticklabel in zip(ax.get_xticks(), ax.get_xticklabels()):
                if float(ticklabel.get_text()) >= float(start_xticklabel):
                    start_x = tick
                    print('Couldnt find start_xticklabel in labels:', start_xticklabel,
                          '\nUsing', ticklabel.get_text(), 'instead.')
                    break

        else:
            raise Exception('Couldnt find start_xticklabel in labels:', start_xticklabel)
        ...
        if find_closest_labels: # look for the closest ticklabel with number smaller than start_xticklabel
            for tick, ticklabel in zip(ax.get_xticks()[::-1], ax.get_xticklabels()[::-1]):
                if float(ticklabel.get_text()) <= float(end_xticklabel):
                    end_x = tick
                    print('Couldnt find end_xticklabel in labels:', end_xticklabel,
                          '\nUsing', ticklabel.get_text(), 'instead.')
                    break
        else:

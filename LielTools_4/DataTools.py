...
def divDFs(df1_numerator , df2_denominator):
...
def getUnsharedComponents(list1, list2):
    ''' Gets 2 lists/series/sets and returns their unshared components '''
    shared = set(list1).intersection(set(list2))
    diff1 = [v for v in set(list1) if v not in shared]
    diff2 = [v for v in set(list2) if v not in shared]
    return [diff1, diff2]


def getSharedComponents(list1, list2, order_matters=False):
    ...
    if not order_matters:
        ...
        if type(list1) is list and type(list2) is list:
            ...
            raise Exception('getSharedComponents: order_matters can be True only if list1 and list2 are both of type list')
...
def isValueSetSame(list1, list2):
...
def assert_column_exists_in_path(file_path, col_name, sheet=0):
    df = FileTools.readExcel(file_path, sheet=sheet)

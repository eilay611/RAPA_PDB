...
def readExcel(path, sheet=0, indexCol=None):
...
def deleteFolder(path, keepFolderItself=True):
    if not os.path.exists(path):
        raise Exception('Folder does not exist!')

    shutil.rmtree(path)
    if keepFolderItself: createFolder(path)
...
def excel2dict(path, sheet=0, colKey=None, colVal=None):
    ...
    info = readExcel(path, sheet=sheet, indexCol=colKey)
...
def get_paths_of_subfolders(folder_path):
    '''
    only one level down.
    :param folder_path:
    :return:
...
def print_subfolders_sizes(folder_path):
    ...
    :return:
    '''
    paths = get_paths_of_subfolders(folder_path)
    for path in paths:

...
def main():
    ...
    parser.add_argument("-PDB_folder", type = str, required = False, default="C:\\Users\\namih\\Documents\\BGU\\SURFMAP-master")
    ...
    # args = parser.parse_args()
    #PDB_folder_path = "/mnt/c/Users/namih/Documents/BGU/SURFMAP-master/"
    ...
    for file in os.listdir(PDB_folder_path):
        ...
        if filename.endswith(".pdb"):
            ...
            pdb = os.path.join(PDB_folder_path,filename)
            run_4_pdb(pdb,"bfactor", True, args)

def run_4_pdb(pdb, tomap, keep, args):
    # python3.10.exe surfmap.py -pdb test.pdb -tomap "bfactor" --keep
    curdir = os.getcwd()
    #absdir = os.path.dirname(os.path.abspath(__file__))
    #absdir = "/mnt/c/Users/namih/Documents/BGU/SURFMAP-master/"
    absdir = ""
    ...
    if pdb:
        ...
        for tomap in listtomap:
            ...
            if args.res:
                ...
                cmd_surfmap = ["python", surftool, "-pdb", pdbarg, "-shell", shell, "-tomap", tomap, "-res", resfile, "-d", outdir]
                cmd_surfmap_s ="python "+surftool +" -pdb "+pdbarg+" -shell "+shell+" -tomap "+tomap+" -res "+resfile+" -d "+outdir
                print (cmd_surfmap_s)
		
                reslist = os.path.splitext(resfile)[0] + "_sph_coords.out"
                print (reslist)
            else:
                cmd_surfmap = ["python", surftool, "-pdb", pdbarg, "-shell", shell, "-tomap", tomap, "-d", outdir]
		
                cmd_surfmap_s ="python "+surftool +" -pdb "+pdbarg+" -shell "+shell+" -tomap "+tomap+" -d "+outdir
                print (cmd_surfmap_s)

            #outsubres = subprocess.call(cmd_surfmap)
            Command.do_command(cmd_surfmap_s)
           ...
           #     exit()
            ...
            #cmdlist = ["Rscript", coordtool, "-f", outdir+"/"+mapfile, "-s", str(cellsize), "-P", proj]
            #cmdlist_s = "Rscript "+ coordtool+ " -f "+ outdir+"/"+mapfile+ " -s "+ str(cellsize)+ " -P "+ proj
            #print (cmdlist)
            #subprocess.call(cmdlist)
            #Command.do_command(cmdlist)
            #subprocess.Popen(cmdlist)
            cmdlist_s = "Rscript  scripts/computeCoordList.R -f " + outdir + "/" + mapfile + " -s 5 -P sinusoidal"
            print (cmdlist_s)
            Command.do_command(cmdlist_s)
            ...
            cmdmat = "Rscript scripts/computeMatrices.R -i "+ outdir+"/coord_lists/"+coordfile+ " -s 5 -P sinusoidal"
            print (cmdmat)
            #subprocess.call(cmdmat)
            Command.do_command(cmdmat)
            ...
            cmdmap = "Rscript scripts/computeMaps.R -i "+ matf +" --bfactor -s 5 -P sinusoidal"
            print(cmdmap)
            Command.do_command(cmdmap)
            exit()


            if not keep: # Deleting all intermediate files and directories

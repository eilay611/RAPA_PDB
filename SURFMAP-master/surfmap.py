    ...
    if args.pdb:
        ...
        for tomap in listtomap:
            ...
            if args.res:
                ...
                cmd_surfmap = ["python3.10.exe", surftool, "-pdb", pdbarg, "-shell", shell, "-tomap", tomap, "-res", resfile, "-d", outdir]
                cmd_surfmap_s ="python3.10.exe "+surftool +" -pdb "+pdbarg+" -shell "+shell+" -tomap "+tomap+" -res "+resfile+" -d "+outdir
                print (cmd_surfmap_s)
		
                reslist = os.path.splitext(resfile)[0] + "_sph_coords.out"
                print (reslist)
            else:
                cmd_surfmap = ["python3.10.exe", surftool, "-pdb", pdbarg, "-shell", shell, "-tomap", tomap, "-d", outdir]
		
                cmd_surfmap_s ="python3.10.exe "+surftool +" -pdb "+pdbarg+" -shell "+shell+" -tomap "+tomap+" -d "+outdir
                print (cmd_surfmap_s)
